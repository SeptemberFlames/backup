...
//桌子规则
#define SR_ALLOW_DRAW_TABLE				0x01000000						//允许绘制桌子
#define SR_ALLOW_YOUKE_ANDRIOD			0x02000000						//允许游客机器
#define SR_ALLOW_MULTI_PLATFROM_CHAT	0x04000000						//允许跨平台聊天
...
public:
	...
	//允许绘制桌子
	static bool IsAllowDrawTable(DWORD dwServerRule) { return (dwServerRule&SR_ALLOW_DRAW_TABLE)!=0; }
	//禁止游客机器
	static bool IsDefendYoukeAndriod(DWORD dwServerRule) { return (dwServerRule&SR_ALLOW_YOUKE_ANDRIOD)!=0; }
	//允许跨平台聊天
	static bool IsAllowMultiPlatfromChat(DWORD dwServerRule) { return (dwServerRule&SR_ALLOW_MULTI_PLATFROM_CHAT) != 0; }
...
public:
	...
	//允许绘制桌子
	static VOID SetAllowDrawTable(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_ALLOW_DRAW_TABLE:dwServerRule&=~SR_ALLOW_DRAW_TABLE; }
	//禁止游客机器
	static VOID SetDefendYoukeAndriod(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_ALLOW_YOUKE_ANDRIOD:dwServerRule&=~SR_ALLOW_YOUKE_ANDRIOD;}
	//允许跨平台聊天
	static VOID SetAllMultiPlatfromChat(DWORD & dwServerRule, bool bEnable) { (bEnable == true) ? dwServerRule |= SR_ALLOW_MULTI_PLATFROM_CHAT : dwServerRule &= ~SR_ALLOW_MULTI_PLATFROM_CHAT; }
};
