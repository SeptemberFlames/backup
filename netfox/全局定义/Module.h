...
bool CTempldateHelper<IModeluInterface>::CreateInstance()
{
	...
	try
	{
		...
		if (CreateProc == NULL)
		{
			_sntprintf_s(m_szDescribe, CountArray(m_szDescribe), TEXT("找不到组件创建函数"));
			return false;
		}
		...
		{
			_sntprintf_s(m_szDescribe, CountArray(m_szDescribe), TEXT("调用函数生成对象失败"));
			return false;
		}
	}
	...
	{
		_sntprintf_s(m_szDescribe, CountArray(m_szDescribe), TEXT("组件创建函数产生未知异常错误，组件创建失败"));
		return false;
	}
