...
//缓冲大小
#define SEND_BUFFER_LEN				4096								//发送大小
#define RECV_BUFFER_LEN				2048								//接收大小
//////////////////////////////////////////////////////////////////////////////////
...
public:
	...
	//连接信息
	virtual bool SetConnectionInfo(DWORD dwDBAddr, WORD wPort, LPCTSTR szDBName, LPCTSTR szUser, LPCTSTR szPassword, bool IsSlave = true) = NULL;
	//连接信息
	virtual bool SetConnectionInfo(LPCTSTR szDBAddr, WORD wPort, LPCTSTR szDBName, LPCTSTR szUser, LPCTSTR szPassword, bool IsSlave = true) = NULL;
...
public:
	//存储过程
	virtual VOID ExecuteProcess(LPCTSTR pszSPName, BOOL bRecordset, DWORD dwTimeout = 0) = NULL;
	//执行语句
	virtual VOID ExecuteSentence(LPCTSTR pszCommand, BOOL bRecordset, DWORD dwTimeout = 0) = NULL;
};
...
#ifdef _UNICODE
#define VER_ISlaveDataBase INTERFACE_VERSION(1,1)
static const GUID IID_ISlaveDataBase = { 0x90CC7F97,0xEA73,0x4B05,0xA9,0x60,0x57,0x31,0x4E,0xA1,0xE5,0xEE };
#else
#define VER_ISlaveDataBase INTERFACE_VERSION(1,1)
static const GUID IID_ISlaveDataBase = { 0xC3ABAB08,0x600E,0x4D86,0xB8,0x11,0x64,0x35,0x56,0xEC,0xE3,0xA4 };
#endif 

//数据库接口
interface ISlaveDataBase : public IUnknownEx
{
	//连接接口
public:
	//打开连接
...
public:
	//存储过程
	virtual VOID ExecuteProcess(LPCTSTR pszSPName, BOOL bRecordset, DWORD dwTimerout = 0) = NULL;
	//执行语句
	virtual VOID ExecuteSentence(LPCTSTR pszCommand, BOOL bRecordset, DWORD dwTimerout = 0) = NULL;
};
...
public:
	...
	//延期请求
	virtual bool PostDataBaseRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize, LONGLONG llConnectIndex) = NULL;
};
...
public:
	//配置端口
	virtual DWORD GetServicePort() = NULL;
	//当前端口
	virtual DWORD GetCurrentPort() = NULL;
...
public:
	...
	//关闭事件
	virtual bool OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, DWORD dwActiveTime, BOOL bClientQuit) = NULL;
	//读取事件
...
public:
	...
	//关闭事件
	virtual bool OnEventTCPNetworkShut(DWORD dwClientAddr, DWORD dwActiveTime, DWORD dwSocketID, BOOL bClientQuit) = NULL;
	//读取事件
...
DECLARE_MODULE_HELPER(SlaveDataBase, KERNEL_ENGINE_DLL_NAME, "CreateSlaveDataBase")
