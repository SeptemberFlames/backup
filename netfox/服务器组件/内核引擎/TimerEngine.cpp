...
bool CTimerThread::OnEventThreadRun()
{
	...
	//获取时间
	DWORD dwTimeSpace = m_dwTimerSpace;
	...
	if ((m_dwLastTickCount != 0L) && (dwNowTickCount > m_dwLastTickCount))
	{
		...
		dwTimeSpace = (dwTimeSpace > dwHandleTickCount) ? (dwTimeSpace - dwHandleTickCount) : 0L;
	}
...
{
	//锁定资源
	CWHDataLocker DataLocker(m_TimerItemSection);
	...
	pTimerItem->wBindParameter = dwBindParameter;
...
bool CTimerEngine::KillTimer(DWORD dwTimerID)
{
	...
	for (INT_PTR i = 0; i < m_TimerItemActive.GetCount(); i++)
	{
		...
		if (pTimerItem->wTimerID != dwTimerID) continue;
...
{
	//锁定资源
	CWHDataLocker DataLock(m_TimerItemSection);
	...
	if (m_dwTimeLeave == 0)
	{
		...
		for (INT_PTR i = 0; i < m_TimerItemActive.GetCount();)
		{
			...
			if (pTimerItem->dwTimeLeave == 0L)
			{
				...
				m_pITimerEngineEvent->OnEventTimer(pTimerItem->wTimerID, pTimerItem->wBindParameter);
