...
#define COLOR_BACK_GROUND				RGB(230,230,230)

//信息颜色
#define COLOR_INFO						RGB(64,0,0)
...
//菜单命令
#define	IDM_CLEAR_ALL					(WM_USER+201)
#define IDM_SELECT_ALL					(WM_USER+202)
#define IDM_COPY_STRING					(WM_USER+203)
#define IDM_DELETE_STRING				(WM_USER+204)
#define IDM_SAVE_STRING					(WM_USER+205)

//常量定义
#define IDT_TRACE_STRING				1
#define WM_TRACE_SERVICE_MESSAGE		(WM_USER+100)
...
//接口查询
VOID * CTraceServiceControl::QueryInterface(REFGUID Guid, DWORD dwQueryVer)
{
...
bool CTraceServiceControl::TraceString(LPCTSTR pszString, enTraceLevel TraceLevel)
{
	...
	_sntprintf_s(TraceInfo.szString, _TRUNCATE, TEXT("%s\n"), pszString);

	//锁定资源
	CWHDataLocker	DataLocker(m_TraceServiceSection);

	//插入队列
	WORD wHeadSize = sizeof(TraceInfo) - sizeof(TraceInfo.szString);
	WORD wDataSize = wHeadSize + CountStringBuffer(TraceInfo.szString);
...
VOID CTraceServiceControl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	FunctionMenu.AppendMenu(MF_SEPARATOR, 0);
...
LRESULT CTraceServiceControl::OnTraceServiceMessage(WPARAM wParam, LPARAM lParam)
{
	...
	_sntprintf_s(szTimeBuffer, _TRUNCATE, TEXT("【%04d-%02d-%02d %02d:%02d:%02d 】"), SystemTime.wYear, SystemTime.wMonth, SystemTime.wDay, SystemTime.wHour, SystemTime.wMinute, SystemTime.wSecond);
	...
	InsertString(pTraceInfo->szString, CharFormat);
