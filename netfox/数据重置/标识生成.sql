-- 功能说明
-------------------------------------------------------------------------
-- 功能说明：实现在不修改现有数据的情况下，对标识做扩展，可以选择保留标识
-------------------------------------------------------------------------
...
-- ID生产 Begin
	...
	BEGIN
		...
		BEGIN
			...
			IF NOT EXISTS (SELECT * FROM ReserveIdentifier WHERE GameID=@BeginGameID)
			BEGIN
		...
		IF @Reserve=0
		BEGIN
			IF NOT EXISTS (SELECT * FROM GameIdentifier WHERE GameID=@BeginGameID)
			BEGIN
...
-- 重新分配
UPDATE AccountsInfo SET AccountsInfo.Gameid=GameIdentifier.Gameid FROM GameIdentifier WHERE AccountsInfo.userid=GameIdentifier.userid
